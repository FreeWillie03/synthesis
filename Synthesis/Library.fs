...
let digits a =		//Create a function ​digits​ to count the number of digits in a number.  The input maybe positive or negative. ​
   ...
   match (t<10)&&(t>-10) with |true -> i+1  |_ -> dig (t/10) (i+1)
	match (a<10)&&(a>-10) with |true -> 1 |_ -> dig (a/10) (1)

let minmax a,b,c,d =		//Create a function ​minmax​ which finds the largest and smallest values out of fourvalues that are provided.
    match (a,b,_,_<_,_,c,d) with	// ​[target: 3 lines]
	|true -> (min a b),(max c d)
	|_ -> (min c d),(max a b)
	

let isLeap a = //Create a function ​isLeap​ which returns true if the given year is a leap year.  
	match (a>1582)&&(a%4=0) with   //Everyyear that is divisible by 4 is a leap year, unless it is also divisible by 100.
	|true -> match (a%100=0)&&(a%400=0) with  //However, if it is also divisible by 400, then it is still a leap year.  
		|true -> true						   //The function should throw an exception of the input year is less than 1582. 
		|_ -> false							   //[target: 5 lines]
	|_ -> false
    

let month _ =	//Create a function ​month​ which accepts an integer between 1 and 12 inclusive, and returns the corresponding month and the number of days in that month,
	failwith "Not implemented"	//assuming that it is ​not​ a leap year.  If an integer less than 1 or greater than 12 is supplied, anexception should be thrown.
								//[target: 13 lines]
   

let toBinary _ =		//Create a function ​toBinary​ which converts a positive integer to a binary string.Throw an exception if a negative integer is supplied.
	failwith "Not implemented"				//[target: 11 lines]
...
let coord _ =		//Create a function ​coord​ which is given a Cartesian coordinate and returns functionsto calculate:
	failwith "Not implemented"	//a.the straight-line distance to another Cartesian coordinate, as calculated by.
